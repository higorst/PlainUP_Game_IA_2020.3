...
def run(
    ...
    RL.update_epsilon(0 if episode == episodes - 1 else (epsilon / 2) if episode > (episodes // 2) else (epsilon / 4) if episode > (episodes // (1/4)) else (epsilon / 6) if episode > (episodes // (1/6)) else epsilon)
    ...
    while game_loop:
        ...
        # add new tree
        tree_group, LAST_TREE=methods.i_have_to_create_another_one(
            tree_group, LAST_TREE)
        # remove tree group
        ...
        if game_loop:
            ...
            if action != 10:
                RL.q_learning(x_plain, y_plain, x_dist, y_dist,
                              x_pos_pass, y_pos_pass, action, state)

            if game_loop:
                action, state=RL.action(
                    x_plain, y_plain, x_dist, y_dist, x_pos_pass, y_pos_pass)
                if action == 0:
                    plain.fly()
            # ---------------------------------------------
